...
class GildedRose
  ...
  ITEM_PROCESSORS = [
    ItemProcessor::Specific.new("Aged Brie", ExpiryStrategy::Normal.new, QualityStrategy.new(min: 0, max: 50, ranges: {
      (MIN_DATE..-1) => QualityStrategy::Delta.new(2),
      (0..MAX_DATE) => QualityStrategy::Delta.new(1)
    })),
    ItemProcessor::Specific.new("Backstage passes to a TAFKAL80ETC concert", ExpiryStrategy::Normal.new, QualityStrategy.new(min: 0, max: 50, ranges: {
      (0..4) => QualityStrategy::Delta.new(2),
      (5..MAX_DATE) => QualityStrategy::Delta.new(1),
      (MIN_DATE..-1) => QualityStrategy::FixedValue.new(0)
    })),
    ItemProcessor::Specific.new("Sulfuras, Hand of Ragnaros", ExpiryStrategy::Eternal.new, QualityStrategy.new(min: 80, max: 80, ranges: {
      (MIN_DATE..MAX_DATE) => QualityStrategy::Delta.new(0),
    }))
  ]
  FALLBACK_PROCESSOR = ItemProcessor::Fallback.new(ExpiryStrategy::Normal.new, QualityStrategy.new(min: 0, max: 50, ranges: {
    (0..MAX_DATE) => QualityStrategy::Delta.new(-1),
    (MIN_DATE..-1) => QualityStrategy::Delta.new(-2)
  }))
